...
public class VentanaPrincipal extends JFrame
{
    ...
    public void mostrarVentanaMapa( )
    {
    	...
    	//analogo a NuevoRestaurante
    	if (ventanaMapa == null || !ventanaMapa.isVisible()) {
    		//ojo que para crear la ventana del mapa necesitamos a la ventaprincipal
    		//pero ademas de los resultanteres
    		ventanaMapa = new VentanaMapa(this, mundo.getRestaurantes(true)); //selecionar a todos los restaurantes
    ...
    public void agregarRestaurante( String nombre, int calificacion, int x, int y, boolean visitado )
    {
    	...
    	Restaurante res = new Restaurante(nombre, calificacion, x, y, visitado);
    	//Paso 2: Agregarlo al diario
    	mundo.agregarRestaurante(res);
    	//Paso 3: actualizar informacion que se muestra, para ello usamos la funcion
    	//actualizarRestaurantes()
    	actualizarRestaurantes();
    ...
    private void actualizarRestaurantes( )
    {
        ...
        //Toca utilizar a PanelLista, para actualizar a los restaurantes que se muestran
        pLista.actualizarRestaurantes(todos);
        //Actualizar al restaurante seleccionado, para eso usamos a pDetalles
        //Vamos a definir que se toma como seleccionado al primero, cuando se actualiza
        if (!todos.isEmpty()) {
        Restaurante primero = todos.getFirst();
